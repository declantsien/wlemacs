...
use objc2_core_text::{CTFont, CTFontCopyPostScriptName};
use objc2_foundation::is_main_thread;
use objc2::rc::Id;

use crate::{font::wr_font_metrics_impl, types::{font, macfont_info, ExternalPtr, WrFontMetrics}};
...
//     let ct_font: Id<CTFont> = unsafe { Id::retain(f.macfont.cast()) }.unwrap();
//     let ps_name = unsafe { CTFontCopyPostScriptName(&ct_font) };
...
pub extern "C" fn wr_add_font(font: *mut font) {
    ...
    let ct_font: Id<CTFont> = unsafe { Id::retain(f.macfont.cast()) }.unwrap();
    let ps_name = unsafe { CTFontCopyPostScriptName(&ct_font) };
    println!("post script name : {ps_name:?}");
    // let f = MacfontInfoRef::new(font as *mut macfont_info);

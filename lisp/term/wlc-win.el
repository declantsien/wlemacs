...
(unless (featurep 'wlc)
  (error "%s: Loading wlc-win.el but not compiled with Wayland client"
         invocation-name))
...
(cl-defmethod window-system-initialization (&context (window-system wlc)
                                            &optional display)
  "Initialize Emacs for wlc frames and open the first connection to an
Wayland server."
  (cl-assert (not wlc-initialized))

  (wlc-open-connection (or display
			       (setq wayland-display-name (or (getenv "WAYLAND_DISPLAY" (selected-frame))
						  (getenv "WAYLAND_DISPLAY"))))
		           nil
		           ;; Exit Emacs with fatal error if this fails and we
		           ;; are the initial display.
		           (eq initial-window-system 'wlc))
...
(cl-defmethod handle-args-function (args &context (window-system wlc))
  (x-handle-args args))

(cl-defmethod frame-creation-function (params &context (window-system wlc))
  (x-create-frame-with-faces params))

;; (cl-defmethod gui-backend-set-selection (selection value
;;                                          &context (window-system wlc))

...
static INSTANCES_CACHE: LazyLock<Mutex<HashMap<(
            FontKey,
            FontSize,
    ...
    FontInstance>>> = LazyLock::new(Default::default);
...
pub extern "C" fn wr_prepara_font(f: *mut frame, font: *mut font) {
    ...
    let hash_key = (key, glyph_size, Some(options), Some(platform_options), variations);
    let font_instance = || {
